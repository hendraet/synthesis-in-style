...
if shutil.which('ninja'):
    ...
    raise RuntimeError('ninja ist not installed')
...
def get_stylegan_1_based_autoencoder(args: argparse.Namespace) -> Callable:
    ...
    encoder_class = WWPlusEncoder if getattr(args, 'w_only', False) else WPlusEncoder
    if getattr(args, 'code_dim', 0) > 0:
...
def get_swagan_generator(image_size, latent_size, n_mlp=8, channel_multiplier=2, init_ckpt=None, ckpt_key='g_ema',
                         strict=True) -> Generator:
    generator = SWAGANGenerator(image_size, latent_size, n_mlp, channel_multiplier=channel_multiplier)
    if init_ckpt is not None:
        load_weights(generator, init_ckpt, key=ckpt_key, strict=strict)
...
def get_swagan_autoencoder(image_size, latent_size, num_input_channels, n_mlp=8, channel_multiplier=2,
                           init_ckpt=None, encoder_class: Union[WPlusEncoder, WWPlusEncoder] = WPlusEncoder,
                           autoencoder_class: Union[
                               StyleganAutoencoder, DropoutStyleganAutoencoder] = StyleganAutoencoder) -> StyleganAutoencoder:
    generator = get_swagan_generator(
        image_size,
        latent_size,
        n_mlp=n_mlp,
        channel_multiplier=channel_multiplier,
...
def get_swagan_based_autoencoder(args: argparse.Namespace) -> Callable:
    encoder_class = WWPlusEncoder if getattr(args, 'w_only', False) else WPlusEncoder
    autoencoder_func = functools.partial(get_swagan_autoencoder, encoder_class=encoder_class)
    return autoencoder_func


def get_autoencoder(config: dict, init_ckpt: str = None) -> Union[StyleganAutoencoder, TwoStemStyleganAutoencoder]:
    assert config['stylegan_variant'] in [1, 2, 'swagan'], "Stylegan Variant Unknown"

    if config['stylegan_variant'] == 1:
        autoencoder_func = get_stylegan_1_based_autoencoder(argparse.Namespace(**config))
    elif config['stylegan_variant'] == 'swagan':
        autoencoder_func = get_swagan_based_autoencoder(argparse.Namespace(**config))
    else:
...
def load_autoencoder_or_generator(args: argparse.Namespace, config: dict) -> Union[StyleganAutoencoder, TwoStemStyleganAutoencoder]:
    autoencoder = get_autoencoder(config).to(args.device)
    # based on the existence of a specific key in our config we decide whether we load the full autoencoder or just
    # the generator
    if 'stylegan_checkpoint' in config:
        autoencoder = load_weights(autoencoder, args.checkpoint, key='autoencoder')
    else:
        autoencoder.decoder = load_weights(autoencoder.decoder, args.checkpoint, key='g_ema')
    return autoencoder
