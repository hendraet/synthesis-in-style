...
def print_result_tables(results: dict):
    ...
    for score_key_filter in score_key_filters:
        ...
        if score_key_filter == "average_iou_scores":
            ...
            params = f"min_confidence {min_confidence} min_contour_area {min_contour_area} patch_overlap {patch_overlap}"
    ...
    print_result_for_given_config(params.split(" "), results)
    ...
    if len(matching_results) == 0:
        ...
        for result in matching_results:
            for k, v in result.items():
                pandas_ready_dict[k].append(v)
        print_data_frame(pandas.DataFrame(data=pandas_ready_dict))
